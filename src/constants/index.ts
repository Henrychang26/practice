...
type tokenSymbolsType = string[];
export const tokenSymbols: tokenSymbolsType = ["ETH", "WETH", "UNI", "USDC"];

type tokenInfosObjectType = {
  address: string;
  decimals: number;
  name: string;
  maxAllowance?: string;
...
type tokenInfosType = {
  [key: string]: tokenInfosObjectType;
};
export const tokenInfos: tokenInfosType = {
  ETH: {
    address: "0x",
    decimals: 18,
    name: "Ether",
...
type outSymbolType = {
  [key: string]: priceTierType;
};

type poolsType = {
  [key: string]: outSymbolType;
};
